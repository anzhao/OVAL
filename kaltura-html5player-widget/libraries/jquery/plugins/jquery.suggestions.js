...
(function ($) {
  $.fn.suggestions = function (param, param2) {
    ...
    function updateSuggestions(delayed) {
      ...
      if (conf._data.timerID != null) clearTimeout(conf._data.timerID);
      if (delayed) setTimeout(doUpdateSuggestions, conf.delay);
      else doUpdateSuggestions();
    }
    ...
    function doUpdateSuggestions() {
      ...
      conf.fetch.call(conf._data.textbox, conf._data.textbox.val());
    }
    ...
    function fitContainer() {
      if (conf._data.div.is(":hidden")) return;

      // FIXME: Mysterious -20 from mwsuggest.js,
      // presumably to make room for a scrollbar
      var availableHeight =
        $("body").height() -
        (Math.round(conf._data.div.offset().top) - $(document).scrollTop()) -
        20;
      ...
      if (numRows < 2 && conf.suggestions.length >= 2) numRows = 2;
      if (numRows > conf.maxRows) numRows = conf.maxRows;
    ...
    function trimResultText() {
      ...
      var fix = 0;
      if (conf._data.visibleResults < conf.suggestions.length) fix = 20;
      //else
      //	fix = operaWidthFix();
      if (fix < 4)
      ...
      if (factor > conf.maxGrowFactor) factor = conf.maxGrowFactor;
      if (factor < 1)
      ...
      if (newWidth != conf._data.div.outerWidth())
        conf._data.div.animate({ width: newWidth }, conf.animationDuration);
      ...
      conf._data.div.find("tr").each(function () {
        ...
        if (span.outerWidth() > newWidth) {
          ...
          while (span.outerWidth() > newWidth && span.text().length > 3) {
            span.text(span.text().substring(0, span.text().length - 4) + "...");
          }
    ...
    function highlightResult(result, updateTextbox) {
      ...
      if (updateTextbox) {
        if (result.size() == 0) restoreText();
        else conf._data.textbox.val(result.data("text"));
      }
      ...
      if (
        ...
        if (to < first)
          ...
          // Need to scroll down
          conf._data.div.scrollTop(
            (to - conf._data.visibleResults + 1) * rowHeight
          );
      }
    }
    ...
    function init() {
      ...
      if (typeof conf.delay == "undefined") conf.delay = 250;
      if (typeof conf.maxGrowFactor == "undefined") conf.maxGrowFactor = 2;
      if (typeof conf.maxRows == "undefined") conf.maxRows = 7;
      if (typeof conf.submitOnClick == "undefined") conf.submitOnClick = false;
      if (typeof conf.suggestions != "object") conf.suggestions = [];
      ...
      $(this)
        ...
        .blur(function () {
          ...
          if (conf._data.mouseDownOn.size() > 0) return;
          conf._data.div.hide();
          cancelPendingSuggestions();
        });
      ...
      conf._data.div
        ...
        .mouseup(function (e) {
          ...
          if (tr.get(0) != other.get(0)) return;

          highlightResult(tr, true);
          conf._data.div.hide();
          conf._data.textbox.focus();
          if (conf.submitOnClick) conf._data.textbox.closest("form").submit();
        });
    }
