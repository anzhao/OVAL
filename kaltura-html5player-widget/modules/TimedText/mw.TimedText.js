  ...
  mw.TimedText.prototype = {
    ...
    init: function (embedPlayer, options) {
      ...
      $j(embedPlayer).bind("onCloseFullScreen onOpenFullScreen", function () {
        ...
        mw.log(
          "TimedText::set text size for: : " +
            embedPlayer.$interface.width() +
            " = " +
      ...
      $j(embedPlayer).bind("onResizePlayer", function (e, size, animate) {
        mw.log(
          "TimedText::onResizePlayer: " +
            _this.getInterfaceSizeTextCss(size)["font-size"]
        );
        if (animate) {
          embedPlayer.$interface
            .find(".track")
    ...
    loadCurrentSubSrouce: function (callback) {
      mw.log(
        "loadCurrentSubSrouce:: enabled source:" + this.enabledSources.length
      );
      for (var i = 0; i < this.enabledSources.length; i++) {
        var source = this.enabledSources[i];
      ...
      if (layoutMode == "ontop") {
        ...
        mw.log(
          " height of " +
            this.embedPlayer.id +
            " is now: " +
            $j("#" + this.embedPlayer.id).height()
        );
      }
      mw.log(
        "should have been appended: " + $playerTarget.find(".track").length
      );
    },
  };
    ...
    load: function (callback) {
      ...
      if (this.textProvider && this.titleKey) {
        this.textProvider.loadTitleKey(this.titleKey, function (data) {
          ...
          mw.log(
            "mw.TimedText:: loaded from titleKey: " +
              _this.captions.length +
              " captions"
          );
      ...
      if (this.getSrc()) {
        ...
        $j.get(
          ...
          function (data) {
            ...
            _this.captions = handler(data);
            mw.log(
              "mw.TimedText:: loaded from srt file: " +
                _this.captions.length +
                " captions"
            );
            // Update the loaded state:
