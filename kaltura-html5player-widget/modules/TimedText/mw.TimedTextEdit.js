...
mw.TimedTextEdit.prototype = {
        ...
        $j("<li>").append(
          $j("<a>")
            ...
            .append(
              ...
              $j("<span>").text(gM("mwe-timedtext-stage-" + edit_stage_id))
            )
        )
      );
      ...
      $j("<div />")
        ...
        .append(
          $j("<span />")
            ...
            .append(
              $j("<h4>").text(gM("mwe-timedtext-upload-text-desc-title")),
              $j("<i>").text(gM("mwe-timedtext-upload-text-desc-help")),
              $j("<ul>").append(
                $j("<li>").text(
                  gM("mwe-timedtext-upload-text-desc-help-browse")
                ),
                $j("<li>").text(
                  gM("mwe-timedtext-upload-text-desc-help-select")
                ),
                $j("<li>").text(
                  gM("mwe-timedtext-upload-text-desc-help-review")
                )
              )
            ),
          //The text preview
          $j("<h3>").text(gM("mwe-timedtext-upload-text-preview")),
          $j('<textarea id="timed-text-file-preview"></textarea>')
        )
    );

    // Adjust the height of the text preview:
    $j("#timed-text-file-preview").css({
      width: "100%",
      height: "300px",
    });
      ...
      $j("<input />")
        ...
        .change(function () {
          ...
          if (mw.Language.names[langKey]) {
            $buttonTarget.find(".btnText").text(mw.Language.names[langKey]);
          }
        }),
    ...
    $j("#timed-text-file-upload").change(function (ev) {
      if ($j(this).val()) {
        ...
        if (file.fileSize > 1048576) {
          $j("#timed-text-file-preview").text(
            "Error the file you selected is too lage"
          );
          return;
        }
    ...
    mw.getToken(targetTitleKey, function (token) {
      ...
      mw.getJSON(request, function (data) {
        ...
        if (data.edit && data.edit.result == "Success") {
          ...
          buttons[gM("mwe-timedtext-upload-text-done-uploading")] =
            function () {
  ...
  getLanguageList: function () {
    ...
    for (var langKey in mw.Language.names) {
      ...
      $langMenu.append(_this.getLangMenuItem(langKey, source_icon));
    }
    return $langMenu;
  },
  ...
  getLangMenuItem: function (langKey, source_icon) {
    return $j.getLineItem(
      ...
      function () {
        ...
        $j("#language-select")
          .find(".btnText")
          .text(mw.Language.names[langKey]);
      }
    );
  },
  /**
