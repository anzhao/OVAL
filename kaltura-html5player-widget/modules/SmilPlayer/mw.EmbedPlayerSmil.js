...
mw.EmbedPlayerSmil = {
  ...
  doEmbedHTML: function () {
    ...
    mw.log(
      "EmbedPlayerSmil::doEmbedHTML: " + this.id + " time:" + this.smilPlayTime
    );
  ...
  monitor: function () {
    ...
    if (
      ...
      this.smilPlayTime >= this.playSegmentEndTime
    ) {
      mw.log(
        "monitor:: Reached playSegmentEndTime pause playback: " +
          this.playSegmentEndTime
      );
    ...
    if (this.isPlaying()) {
      ...
      if (
        syncDelta != 0 &&
        (syncDelta > this.maxSyncDelta || this.pausedForBuffer)
      ) {
        mw.log(
          "EmbedSmilPlayer:: monitor: syncDelta too large buffering: " +
            syncDelta
        );
