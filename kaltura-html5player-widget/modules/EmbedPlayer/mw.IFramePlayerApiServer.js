...
(function (mw) {
  ...
  mw.IFramePlayerApiServer.prototype = {
    ...
    getParentUrl: function () {
      ...
      if (!purl) {
        mw.log(
          "Error: iFramePlayerApiServer:: could not parse parent url. \n" +
    ...
    postMessage: function (msgObject) {
      ...
      $j.postMessage(messageString, this.getParentUrl(), window.parent);
    },
    ...
    hanldeMsg: function (event) {
      ...
      if (!mw.getConfig("EmbedPlayer.EnableIframeApi")) {
        mw.log(
          "Error: Loading iFrame playerApi but config EmbedPlayer.EnableIframeApi is false"
        );
        return false;
      }
      ...
      if (!this.eventDomainCheck(event.origin)) {
        mw.log(
          "Error: " +
            event.origin +
            " domain origin not allowed to send player events"
        );
        return false;
      }
      // Decode the message
      ...
      if (msgObject.method && this.embedPlayer[msgObject.method]) {
        this.embedPlayer[msgObject.method].apply(
          this.embedPlayer,
          $j.makeArray(msgObject.args)
        );
      }
      // Update a attribute
      if (
        typeof msgObject.attrName != "undefined" &&
        typeof msgObject.attrValue != "undefined"
      ) {
        try {
    ...
    eventDomainCheck: function (origin) {
      if (mw.getConfig("EmbedPLayer.IFramePlayer.DomainWhiteList")) {
        ...
        var domainWhiteList = mw.getConfig(
