    ...
    mw.setupUserConfig(function () {
      ...
      $j(playerSelect).each(function (index, playerElement) {
        ...
        if ($j(playerElement).hasClass("nativeEmbedPlayerPid")) {
          ...
          mw.log(
            "EmbedPlayer::$j.embedPlayer skip embedPlayer gennerated video: " +
              playerElement
          );
        } else {
...
EmbedPlayerManager.prototype = {
  ...
  addPlayerElement: function (playerElement, attributes) {
    ...
    mw.load(playerDependencyRequest, function () {
      ...
      if (waitForMeta && mw.getConfig("EmbedPlayer.WaitForMeta")) {
        mw.log(
          "EmbedPlayer::WaitForMeta ( video missing height (" +
            $j(playerElement).attr("height") +
            "), width (" +
            $j(playerElement).attr("width") +
            ") or duration: " +
  ...
  swapEmbedPlayerElement: function (targetElement, playerInterface) {
    ...
    for (var method in playerInterface) {
      if (method != "readyState") {
        // readyState crashes IE ( don't include )
  ...
  playerReady: function (player) {
    ...
    if (is_ready) {
      ...
      mw.log(
        "EmbedPlayer::All on-page players ready run playerManager callbacks"
      );
  ...
  autoSelectSource: function () {
    ...
    for (var source = 0; source < playableSources.length; source++) {
      ...
      if (
        ...
        mw.log(
          "EmbedPlayer::autoSelectSource: Set via preference: " +
    ...
    for (var source = 0; source < playableSources.length; source++) {
      if (playableSources[source].markedDefault) {
        mw.log(
          "EmbedPlayer::autoSelectSource: Set via marked default: " +
    ...
    for (var source = 0; source < playableSources.length; source++) {
      ...
      if (
        ...
        mw.log(
          "EmbedPlayer::autoSelectSource: Set h264 via native or flash fallback"
        );
        setSelectedSource(playableSources[source]);
        return true;
      }
    ...
    if (!this.selectedSource) {
      mw.log(
        "EmbedPlayer::autoSelectSource: Set via first source:" +
          playableSources[0]
      );
...
mw.EmbedPlayer.prototype = {
  ...
  init: function (element, customAttributes) {
    ...
    mw.log(
      "EmbedPlayer::mediaElement:" + this.id + " duration is: " + this.duration
    );
  ...
  loadPlayerSize: function (element) {
    ...
    mw.log(
      "EmbedPlayer::loadPlayerSize: css size:" +
        this.width +
        " h: " +
        this.height
    );
  ...
  checkPlayerSources: function () {
    ...
    if (_this.apiTitleKey && this.mediaElement.sources.length == 0) {
      ...
      mw.log(
        "EmbedPlayer::checkPlayerSources: loading apiTitleKey:" +
          _this.apiTitleKey
      );
  ...
  switchPlaySrc: function (source) {
    mw.log(
      "Error: only native playback supports insertAndPlaySource right now"
    );
  },
  ...
  setupSourcePlayer: function () {
    mw.log(
      "EmbedPlayer::setupSourcePlayer: " +
        this.id +
        " sources: " +
        this.mediaElement.sources.length
    );
  ...
  inheritEmbedPlayer: function (callback) {
    mw.log(
      "EmbedPlayer::inheritEmbedPlayer:duration is: " +
        this.getDuration() +
        " p: " +
        this.id
    );
    ...
    mw.log(
      "EmbedPlayer::inheritEmbedPlayer: embedding with " +
        this.selectedPlayer.library
    );
    var _this = this;
    ...
    this.selectedPlayer.load(function () {
      mw.log(
        "EmbedPlayer::inheritEmbedPlayer " +
  ...
  doSeek: function (percent) {
    ...
    if (this.supportsURLTimeEncoding()) {
      mw.log(
        "EmbedPlayer::doSeek:: updated serverSeekTime: " +
  ...
  setCurrentTime: function (time, callback) {
    mw.log(
      "Error: base embed setCurrentTime can not frame serve (override via plugin)"
    );
  },
    ...
    if (mw.isIpad()) {
      if (
        ...
        if (mw.getConfig("EmbedPlayer.EnableIpadHTMLControls")) {
          mw.log(
            "Error:: Trying to set EnableIpadHTMLControls without Persistent Native Player"
          );
        }
        return true;
      }
  ...
  showNativePlayer: function () {
    ...
    if (!source || !source.src) {
      mw.log(
        "Warning: Your probably fakeing the iPhone userAgent ( no h.264 source )"
      );
  ...
  monitor: function () {
    ...
    if (this.currentTime >= 0 && this.duration) {
      ...
      if (this.currentTime >= endPresentationTime) {
        mw.log(
          "mWEmbedPlayer::should run clip done :: " +
            this.currentTime +
            " > " +
            endPresentationTime
        );
  ...
  getPlayerElementTime: function () {
    mw.log(
      "Error: getPlayerElementTime should be implemented by embed library"
    );
  },
  ...
  supportsURLTimeEncoding: function () {
    ...
    if (timeUrls == "none") {
      ...
      mw.log(
        "Error:: invalid config value for EmbedPlayer.EnableURLTimeEncoding:: " +
          mw.getConfig("EmbedPlayer.EnableURLTimeEncoding")
      );
    }
    return false;
  },
  ...
  setPlayerPreference: function (playerId, mimeType) {
    ...
    for (var i = 0; i < this.players.length; i++) {
      if (this.players[i].id == playerId) {
        ...
        mw.log(
          "EmbedPlayer::setPlayerPreference: choosing " +
            playerId +
            " for " +
            mimeType
        );
    ...
    if (selectedPlayer) {
      ...
      for (var i = 0; i < playerList.length; i++) {
        ...
        if (
          ...
          mw.log(
            "EmbedPlayer::setPlayerPreference: using " +
              embed.selectedPlayer.getName() +
              " for " +
