...
(function (mw) {
  ...
  mw.loader = {
    ...
    load: function (loadRequest, instanceCallback) {
      ...
      if (this.moduleLoaders[loadRequest]) {
        ...
        if (!resourceSet) {
          mw.log(
            "mw.load:: Error with module loader: " +
              loadRequest +
              " ( no resource set defined )"
          );
          return;
        }
      ...
      if (loadRequest) {
        ...
        if (loadRequest.indexOf(".js") == -1 && !mw.getResourceLoaderPath()) {
          mw.log(
            "Error: are you sure " +
              loadRequest +
              " is a file ( is it missing a resource path? ) "
          );
        }
    ...
    loadResource: function (resourceName, callback) {
      ...
      if (this.resourceStyleDependency[resourceName]) {
        if (!mw.isset(this.resourceStyleDependency[resourceName])) {
          mw.log(
            "loadResource:: dependent css resource: " +
  ...
  mw.addDialog = function (options) {
    ...
    if (!options.title || !options.content) {
      mw.log(
        "Error: mwEmbed addDialog missing required options ( title, content ) "
      );
      return;
    }
  ...
  mw.getMwEmbedSrc = function () {
    ...
    for (var i = 0; i < js_elements.length; i++) {
      ...
      if (src) {
        if (
          // Check for mwEmbed.js ( debug mode )
          (src.indexOf("mwEmbed.js") !== -1 &&
            src.indexOf("MediaWiki:Gadget") == -1) || // Check for resource loader
          ((src.indexOf("mwResourceLoader.php") !== -1 ||
            src.indexOf("ResourceLoader.php") !== -1) &&
            src.indexOf("mwEmbed") !== -1) || // Check for static mwEmbed package
  ...
  mw.styleRuleExists = function (styleRule) {
    ...
    for (var i = 0; i < document.styleSheets.length; i++) {
      ...
      try {
        ...
        mw.log(
          "Error: cant check rule on cross domain style sheet:" +
    ...
    (function ($) {
      ...
      $.fn.triggerQueueCallback = function (
        ...
        if (!callbackSet || callbackSet.length === 0) {
          mw.log(
            '"mwEmbed::jQuery.triggerQueueCallback: No events run the callback directly: ' +
              triggerName
          );
          // No events run the callback directly
          callback();
...
if (mw.getResourceLoaderPath() && !window.jQuery) {
  mw.log(
    "Error: jQuery is required for mwEmbed, please update your resource loader request"
  );
}
